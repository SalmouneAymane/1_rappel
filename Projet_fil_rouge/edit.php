...
$courseStatement = $pdo->prepare('SELECT * FROM courses WHERE id = :id');
            ...
            $statement = $pdo->prepare(
                'UPDATE courses
                 SET title = :title, slug = :slug, description = :description,
                     teacher_id = :teacher_id, subject_id = :subject_id, status = :status
                 WHERE id = :id'
            );
            $statement->execute([
                'title' => $values['title'],
...
<body class="min-h-screen bg-slate-100 text-slate-900">
    <main class="mx-auto max-w-3xl px-4 py-10 sm:px-6">
        ...
        <div class="mt-4 rounded-xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
            ...
            <p class="mt-2 text-slate-600">Update the course information and publication status.</p>
            ...
            <form method="post" class="mt-6 space-y-5">
                ...
                <div>
                    ...
                    <input id="slug" name="slug" value="<?= e($values['slug']) ?>" required class="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2.5 focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-200">
                </div>
                <div>
                ...
                <div class="flex justify-end gap-3 pt-2">
                    ...
                    <button type="submit" class="rounded-lg bg-indigo-600 px-4 py-2.5 font-semibold text-white hover:bg-indigo-700">Save Changes</button>
                </div>
            </form>
        </div>
    </main>
